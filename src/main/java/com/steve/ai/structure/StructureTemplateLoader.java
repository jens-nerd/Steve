...
public class StructureTemplateLoader {
    ...
    public static LoadedTemplate loadFromNBT(ServerLevel level, String structureName) {
        ...
        for (String fileName : possibleNames) {
            ...
            if (resourceStream != null) {
                ...
                try {
                    CompoundTag nbt = NbtIo.readCompressed(resourceStream, NbtAccounter.unlimitedHeap());
    ...
    private static LoadedTemplate parseNBTStructure(CompoundTag nbt, String name) {
        ...
        var sizeList = nbt.getListOrEmpty("size"); // TAG_Int list
        int width = sizeList.getIntOr(0, 0);
        int height = sizeList.getIntOr(1, 0);
        int depth = sizeList.getIntOr(2, 0);

        var paletteList = nbt.getListOrEmpty("palette"); // TAG_Compound list
        ...
        for (int i = 0; i < paletteList.size(); i++) {
            ...
            try {
                Identifier blockLocation = Identifier.parse(blockName);
                Block block = net.minecraft.core.registries.BuiltInRegistries.BLOCK.getValue(blockLocation);
